  ...
  local function walk_waitd(waitd)
    for taskd, _ in pairs(waitd.tasks) do
      ...
      if taskd.waitingfor == waitd and taskd.status=='ready' then
        ...
        step_task(taskd, event, parameter)
        ...
        waitd.buff_mode == 'keep_first' and not waitd.buff_event then
          waitd.buff_event, waitd.buff_parameter = event, parameter
        end
      end
    end
  end
		...
		local check = function(ok, ...)
			if coroutine.status(taskd.co)=='dead' then
				...
				if ok then 
					...
					emit_signal(taskd.EVENT_FINISH, {...}) --per task
					emit_signal(M.EVENT_FINISH, {taskd=taskd, ...}) --global
				else
					--log('SCHED', 'WARNING', '%s die on error, returning %d parameters: %s'
					--	, tostring(taskd), select('#',...), tostring(...))
          emit_signal(taskd.EVENT_DIE, {...}) --per task
					emit_signal(M.EVENT_DIE, {taskd=taskd, ...}) --global
				end
				taskd.status='dead'
			end
		end
...
M.kill = function ( taskd )
  ...
  emit_signal(taskd.EVENT_DIE, {'killed'}) --per task
  emit_signal(M.EVENT_DIE, {taskd=taskd, 'killed'}) --global
	taskd.status = 'dead'
end
...
M.step = function ()
  ...
  --if find one waiting&timeouting, wake it --and finish
	for taskd, _ in pairs (m_tasks) do
		if taskd.status=='ready' then
